...
const ChatModule = {
    ...
    try {
        ...
        // ✅ Detect player join/leave from broadcast messages
        if (msg.action !== 'register_response' && 
            msg.action !== 'login_response' && 
            msg.action !== 'poll_response') {
            
            // Detect new player messages (first message from user)
            if (msg.name && 
                msg.name !== this.currentUser && 
                !msg.isServer && 
                !this.processedMessageIds.has(msg.id)) {
                
                const isFirstMessage = !Array.from(this.processedMessageIds).some(id => {
                    // Check if we've seen this username before
                    const messages = document.querySelectorAll('.message.other .sender-name');
                    return Array.from(messages).some(el => el.textContent === msg.name);
                });
                
                if (isFirstMessage) {
                    window.SystemNotifications?.playerJoined(msg.name);
                }
            }
        }
    ...
    // ✅ Client-side commands (process locally, don't send to server)
    if (this.handleClientCommand(trimmed)) {
        return true;
    }
...
// ✅ NEW: Handle client-side commands
handleClientCommand(command) {
    ...
    switch (cmd.toLowerCase()) {
        ...
        case '/logout':
        case '/exit':
            this.displayServerMsg('👋 Đang đăng xuất... / Logging out...', true);
            setTimeout(() => window.LoginModule?.logout(), 500);
            return true;
            
        default:
            return false; // Not a client command, send to server
    }
},

// ✅ NEW: Measure WebSocket ping
measurePing() {
    if (this.ws?.readyState !== WebSocket.OPEN) {
        this.displayServerMsg('❌ Chưa kết nối / Not connected', true);
        return;
    }
    ...
    this._pingStart = startTime;
    
    // Send ping (using poll action as heartbeat)
    this.ws.send(JSON.stringify({ 
        action: 'poll',
        _ping: pingId 
    }));
    ...
    const timeout = setTimeout(() => {
        this.displayServerMsg('⏱️ Ping timeout (>5000ms)', true);
    ...
    this.ws.onmessage = (e) => {
        ...
        this.displayServerMsg(
            ...
            `📡 WebSocket: ${this.ws?.readyState === 1 ? 'OPEN' : 'CLOSED'}\n` +
            `🔗 Server: ${this.currentServerUrl}`,
            true
        );
        ...
        DEBUG.info('PERF', 'Ping measured', { latency, quality });
    };
},
...
🇻🇳 Tiếng Việt:
   ...
   • Thu thập tài nguyên (fish/mine/chop), chế tạo trang bị và nâng cấp!
...
🇬🇧 English:
   ...
   • Gather resources (fish/mine/chop), craft equipment and level up!

💡 Client Commands: /ping (check latency) | /quit (logout)
`.trim()
};
...
const SystemNotifications = {
    show(text, duration = 3000) {
        ...
        notif.style.cssText = `
            ...
            background: rgba(0, 0, 0, 0.85); color: white; padding: 16px 32px;
            border-radius: 12px; font-size: 14px; font-weight: 500; z-index: 10001;
            box-shadow: 0 4px 20px rgba(0,0,0,0.5); backdrop-filter: blur(10px);
            border: 2px solid rgba(255, 255, 255, 0.1);
            animation: systemNotifFadeIn 0.3s ease-out;
            pointer-events: none; text-align: center; min-width: 200px;
        `;
        notif.textContent = text;
        
        // Inject animation if not exists
        if (!document.getElementById('systemNotifStyle')) {
            const style = document.createElement('style');
            style.id = 'systemNotifStyle';
            style.textContent = `
                @keyframes systemNotifFadeIn {
                    from { opacity: 0; transform: translate(-50%, -60%); }
                    to { opacity: 1; transform: translate(-50%, -50%); }
                }
                @keyframes systemNotifFadeOut {
        ...
        setTimeout(() => {
            notif.style.animation = 'systemNotifFadeOut 0.3s ease-in';
            setTimeout(() => notif.remove(), 300);
        }, duration);
        
        DEBUG.info('NOTIF', text);
    },
    
    playerJoined(username) {
        this.show(`🟢 ${username} đã vào game / ${username} joined the game`);
    },
    
    playerLeft(username) {
        this.show(`🔴 ${username} đã thoát game / ${username} left the game`);
    }
};
...
window.SystemNotifications = SystemNotifications; // ✅ Add this
window.sendMessage = () => {
    const input = Utils.getEl('messageInput');
