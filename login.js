    ...
    loginWithGmail() {
        if (typeof google === 'undefined') {
            ...
            setTimeout(() => {
                if (typeof google !== 'undefined') {
                    ...
                    this.setAuthMessage('❌ Không thể tải Google Sign-In!', 'error');
                }
            }, 1000);
            return;
        }
        ...
        google.accounts.id.prompt((notification) => {
            if (notification.isNotDisplayed() || notification.isSkippedMoment()) {
                DEBUG.warn('AUTH', 'Gmail prompt not shown', notification.getNotDisplayedReason());
            }
        });
    },
...
window.loginWithGmail = () => LoginModule.loginWithGmail();
...
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => {
        setTimeout(() => LoginModule.initGoogleSignIn(), 500);
    });
} else {
    setTimeout(() => LoginModule.initGoogleSignIn(), 500);
}